...
- **🔄 直连与本地热切双模式**：
  ...
  - **本地代理热切模式 (Proxy Hot-Switch)**：常驻后台守护进程（`127.0.0.1:8899`），拦截大模型流式请求，通过 HTTP IPC 控制面实现**亚毫秒级动态路由热更**，终端开发无需重启会话。
...
└──────────────┬───────────────────────────────┬──────────────┘
               ...
               ▼ (~/.z-switch/providers.json)  ▼ (POST http://127.0.0.1:8899/_admin/switch)
┌─────────────────────────┐         ┌─────────────────────────────────────────┐
...
│ (请求 127.0.0.1:8899)   │ (零缓冲)│ │  • /v1/messages                     │ │
│                         │         │ │  • /v1/chat/completions             │ │
└─────────────────────────┘         │ └──────────────────┬──────────────────┘ │
                                    └────────────────────┼────────────────────┘
                                                         ▼ 向上游透传并流式返回
